function scr_config_load()
{
	if (file_exists(game_save_id + "save_file/config.ini")) /* Check if config.ini exists before loading any settings */
	{
		...
		if (ini_key_exists("config", "language_localization")){global.language_localization = ini_read_real("config", "language_localization", 0);} /* Load language before loading font, as the font is informed by the language selected */
