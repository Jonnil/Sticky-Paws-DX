	...
	(place_meeting(x, y, obj_wall_climb_panel))) {
		if (!on_ground && !climb && !horizontal_rope_climb && vspeed >= 0 && !place_meeting(x, y + 32, obj_wall)) {
		    if ((key_left_hold && !key_right_hold && wall_jump_setting >= 1 && place_meeting(x - 1, y, obj_wall)) ||
		        (!key_right_hold && !dive && wall_jump_setting == 1 && image_xscale < 0 && place_meeting(x - 1, y, obj_wall)) ||
		        (dive && wall_jump_setting == 1 && image_xscale < 0 && place_meeting(x - 1, y, obj_wall)) ||
		        ...
		        if (!crouch && !ground_pound && !ledge_grab && !burnt) {
		            ...
		            triplejumpdelay = 0;
		             ...
		             (!key_left_hold && !dive && wall_jump_setting == 1 && image_xscale > 0 && place_meeting(x + 1, y, obj_wall)) ||
		             (dive && wall_jump_setting == 1 && image_xscale > 0 && place_meeting(x + 1, y, obj_wall)) ||
		...
		if (stick_to_wall) {
		    ...
		    if (!crouch && !ground_pound && !ledge_grab) {
		        ...
		        if ((image_xscale < 0 && !place_meeting(x - 1, y, obj_wall)) ||
		            (image_xscale > 0 && !place_meeting(x + 1, y, obj_wall))) {
		            stick_to_wall = false;
					wall_climb_slippy = 0;
		            audio_stop_sound(snd_skiddingvertical);
		        }
		    ...
		    if ((key_jump && !on_ground && place_meeting(x + 1, y, obj_wall)) ||
		        (key_jump && !on_ground && place_meeting(x - 1, y, obj_wall)) ||
