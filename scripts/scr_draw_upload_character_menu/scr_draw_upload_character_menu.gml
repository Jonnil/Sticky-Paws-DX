function scr_draw_upload_character_menu()
{
	...
	{
		var character_zip_path = game_save_id + "uploads/" + string(character_id) + ".zip";
		show_debug_message("[upload_character] tick delay=" + string(menu_delay) + " exists=" + string(file_exists(character_zip_path)) + " path=" + character_zip_path);
		if (global.username == "")
		{
			keyboard_string = "";
			menu_delay = 3;
		...
		if (menu_delay <= 40)
		&& (!file_exists(character_zip_path))
		{
	...
	&& !file_exists(character_zip_path))
	{
		show_debug_message("[upload_character] menu_delay==40 creating zip: " + character_zip_path);
	...
	#region /* Send Zip File to the Server */
		...
		&& (file_exists(character_zip_path))
	{
		show_debug_message("[upload_character] menu_delay==0 found zip, preparing upload: " + character_zip_path);
		var zip_file = file_bin_open(character_zip_path, 0);
		...
		zip_megabytes = zip_size / 1024 / 1024;
			...
			if (zip_megabytes > global.max_file_upload_megabytes)
			{
				...
				|| (GM_build_type == "exe")
				{
					file_delete(character_zip_path);
				}
						...
						if (global.switch_logged_in)
						{
							if (global.switch_account_network_service_available) /* Need to make sure that network service is available before going online */
							{
								if (scr_online_token_is_valid() == true)
								{
									...
									file_name = filename_name(character_zip_path);
									...
									buffer_load_ext(send_buffer, character_zip_path, 0);
	...
	&& (!file_exists(character_zip_path))
	{
		show_debug_message("[upload_character] menu_delay==0 zip missing, resetting delay to 45. Path: " + character_zip_path);
		menu_delay = 45;
	}
	...
	|| (menu == "character_uploaded_discord")
	{
		...
		#region /* After uploading a character, let player know that there is a Discord server for the game */
		if (global.enable_option_for_pc)
		{
			...
			draw_menu_button_sprite(spr_menu_button, 32, community_discord_y, 0, 0, 2.1, 1, 370 * 2.1, 42, string(global.link_to_discord), "character_uploaded_discord", "character_uploaded_discord", false);
			...
			scr_draw_text_outlined(32, get_window_height - 32 * 4, l10n_text("Share and discover characters"), global.default_text_size, c_black, c_white, 1);
			scr_draw_text_outlined(32, get_window_height - 32 * 3, l10n_text("Discuss character design"), global.default_text_size, c_black, c_white, 1);
			...
			{
				menu_delay = 3;
				if (menu == "character_uploaded_discord")
				{
					menu = "character_uploaded";
				}
			}
		...
		#endregion /* After uploading a character, let player know that there is a Discord server for the game END */
