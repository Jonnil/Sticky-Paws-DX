function scr_initialize_character_sprite(sprite_name, character_sprite_variable = noone, use_character_folder = character_folder)
{
	...
	if (character_sprite_variable != noone
	&& sprite_found)
	{
		...
		if (spr_origin_x == -1) /* If the x origin is not found in the config file, set it to half the sprite width and save it to the config file */
		{
			...
			{
				if (string_pos("custom_characters", character_folder) > 0) /* Includes the string "custom_characters" */
				{
					ini_write_real("sprite origin points", "sprite_" + string(sprite_name) + "_xorig", spr_origin_x);
				}
				else
				{
					show_debug_message("[scr_initialize_character_sprite] WARNING: 'can_save_to_character_config' is set to true, but 'character_folder' is still set to 'custom_characters' buring writing to xorig. Sprite Name: " + string(sprite_name));
				}
			}
		}
		if (spr_origin_y == -1) /* If the y origin is not found in the config file, set it to half the sprite height and save it to the config file */
		{
			...
			if (can_save_to_character_config)
			{
				...
				{
					show_debug_message("[scr_initialize_character_sprite] WARNING: 'can_save_to_character_config' is set to true, but 'character_folder' is still set to 'custom_characters' buring writing to yorig. Sprite Name: " + string(sprite_name));
				}
			}
		}
