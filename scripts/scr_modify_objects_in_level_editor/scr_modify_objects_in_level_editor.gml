function scr_modify_objects_in_level_editor()
{
	...
	&& (!mouse_check_button(mb_middle))
	&& (!mouse_check_button(mb_right))
	&& (asset_get_type("obj_leveleditor") == asset_object)
	&& (instance_exists(obj_leveleditor))
	&& (obj_leveleditor.fill_mode == false)
	&& (obj_leveleditor.erase_mode == false)
	&& (obj_leveleditor.pause == false)
	&& (obj_leveleditor.menu_delay == 0)
	&& (asset_get_type("obj_leveleditor_placed_object") == asset_object)
	&& (asset_get_type("obj_level_player1_start") == asset_object)
	&& (!place_meeting(x, y, obj_level_player1_start))
	&& (asset_get_type("obj_level_player2_start") == asset_object)
	&& (!place_meeting(x, y, obj_level_player2_start))
	&& (asset_get_type("obj_level_player3_start") == asset_object)
	&& (!place_meeting(x, y, obj_level_player3_start))
	&& (asset_get_type("obj_level_player4_start") == asset_object)
	&& (!place_meeting(x, y, obj_level_player4_start))
	&& (asset_get_type("obj_level_height") == asset_object)
	&& (!position_meeting(x, y, obj_level_height))
	&& (asset_get_type("obj_level_width") == asset_object)
	&& (!position_meeting(x, y, obj_level_width))
	&& (asset_get_type("obj_water_level_height") == asset_object)
	&& (!position_meeting(x, y, obj_water_level_height))
	&& (!point_in_rectangle(mouse_get_x, mouse_get_y, 0, display_get_gui_height() - 64, obj_leveleditor.always_show_level_editor_buttons_x + 32, room_height * 2)) /* Can't place objects when clicking the bottom buttons */
	...
	&& (obj_leveleditor.placing_object <= 0)
	{
		...
		&& (!point_in_rectangle(mouse_get_x, mouse_get_y, display_get_gui_width() - 32 - 32 - 32, 80 + obj_leveleditor.icons_at_top_y + 16 - 32 - 32, display_get_gui_width() + 64 + 32, 80 + obj_leveleditor.icons_at_top_y + 16 + 32 + 32)) /* Up and down buttons when grid is on */
		|| (obj_leveleditor.show_grid == false)
		{
