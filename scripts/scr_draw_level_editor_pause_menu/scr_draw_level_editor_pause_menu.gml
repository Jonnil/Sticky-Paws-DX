function scr_draw_level_editor_pause_menu()
{
	...
	&& (pressing_play_timer == 0)
	{
		...
		|| (menu == "quit")
		{
			...
			if (if_daily_build)
			{
				daily_build_y = clear_check_y + 32;
				draw_set_halign(fa_left);
				draw_set_valign(fa_middle);
		...
		|| (menu == "generate_level_map_no")
		{
			...
			if (global.enable_option_for_pc)
			{
				...
				var file_path_text = string_replace_all(scr_censor_game_save_id_for_display(game_save_id + "\custom_levels\\full_level_map_" + string(global.level_name) + ".png"), "\\", "/");
