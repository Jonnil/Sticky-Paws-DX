function scr_set_show_controls_y(from_y_position = 22, always_show_controls = false)
{
	...
	&& (os_type != os_android)
	{
		...
		{
			if (player_show_controls_alpha[i] > 0
		...
		for(var i = 1; i <= global.max_players; i += 1)
		{
			...
			if (player_show_controls_alpha[i] > 0)
			{
				/* If player show controls alpha is more than 0, then change the y position to correct position */
			...
			{
				/* If player show controls alpha is not more than 0, then hide the hud by setting it to +34 */
