...
function scr_language_pack_update(forced = false)
{
	...
	/*	We'll store it under the [language_updates] section, key: "language_last_update_time" */
	ini_open(game_save_id + "save_file/config.ini");
	var language_last_update_time = ini_read_real("language_updates", "language_last_update_time", 0);
	ini_close();
	
	/* If we never stored it before, set it to an old date/time so that the next check can trigger */
	if (language_last_update_time == 0)
	{
		/* For convenience, pick an old date so the script sees it as "very old" */
		language_last_update_time = date_create_datetime(1971, 1, 1, 0, 0, 0);
	}
	...
	var diff_in_seconds = date_second_span(language_last_update_time, language_current_time);
	...
	scr_download_language_packs(); /* <-- Replace with your actual update logic */
	
	/* 7. Record the new "language last update time" */
	ini_open(game_save_id + "save_file/config.ini");
	ini_write_real("language_updates", "language_last_update_time", date_current_datetime());
	ini_close();
}
