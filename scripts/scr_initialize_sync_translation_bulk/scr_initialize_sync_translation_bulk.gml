...
function scr_initialize_sync_translation_bulk(translation_keys)
{
	...
	{
		show_debug_message("[TranslationSync] Error: No translation keys to sync.");
		return false;
	}
	...
	if (request_id == -1)
	{
		show_debug_message("[TranslationSync] Error: Failed to send bulk HTTP POST request.");
		return false;
	}
	else
	{
		show_debug_message("[TranslationSync] Bulk HTTP request sent successfully. Request ID: " + string(request_id));
		return request_id;
	}
}
