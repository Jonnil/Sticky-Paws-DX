...
if (_is_primary_request)
{
	...
	var _search_download_active = (variable_instance_exists(self, "search_for_id_still") && search_for_id_still);
	...
	if (status_code == 200)
	{
		...
		if (response_json != undefined)
		{
			...
			|| _has_base64)
			{
				...
				/* For search-download flow, always save to the deterministic expected path. */
				if (_search_download_active
				&& variable_global_exists("expected_download_zip_path")
				&& global.expected_download_zip_path != "")
				{
					file_save_location = normalize_path_seps(global.expected_download_zip_path);
				}
				else
				{
					/* Build filename for non-search flows */
					var _fname = "";
					if (_has_name)
					{
			...
			{
				/* Search download expects file payload; do not treat list payload as success. */
				if (_search_download_active)
				{
					scr_log("ERROR", "HTTP.PRIMARY", "missing_file_payload", "reason=no_name_and_no_base64");
					search_for_id_still = false;
					in_online_download_list_menu = false;
					in_online_download_list_load_menu = false;
					global.download_request_timeout_end = undefined;
					global.download_failure_reason = "missing_payload";
					global.expected_download_zip_path = "";
				...
				&& buffer != -1)
				{
					...
					if (file_exists(file_save_location))
					{
						...
						if (_search_download_active)
						{
							...
							global.expected_download_zip_path = file_save_location;
						}
					...
					else
					{
						...
						if (_search_download_active)
						{
							...
							in_online_download_list_load_menu = false;
							global.download_request_timeout_end = undefined;
							global.download_failure_reason = "save_failed";
							global.expected_download_zip_path = "";
							menu = "searched_file_downloaded_failed";
							menu_delay = 3;
						}
		...
		else
		{
			...
			if (_search_download_active)
			{
				...
				in_online_download_list_load_menu = false;
				global.download_request_timeout_end = undefined;
				global.download_failure_reason = "invalid_json";
				global.expected_download_zip_path = "";
				menu = "searched_file_downloaded_failed";
				menu_delay = 3;
			}
	...
	else
	{
		...
		if (_search_download_active)
		{
			...
			in_online_download_list_load_menu = false;
			global.download_request_timeout_end = undefined;
			global.download_failure_reason = "http_non_200";
			global.expected_download_zip_path = "";
			menu = "searched_file_downloaded_failed";
			menu_delay = 3;
		}
