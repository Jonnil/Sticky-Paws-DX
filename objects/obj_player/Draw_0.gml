...
var view_center_y = view_y + view_height;
...
|| (y > view_center_y && y < room_height) /* Outside bottom view */ {
...
&& (y < view_y + (view_height * 0.5)) {
	...
	draw_circle_color(x, view_y + 32, 24, c_black, c_black, true);
	draw_circle_color(x, view_y + 32, 22, c_white, c_white, true);
	draw_set_alpha(1);
	
	if (!assist_invincible && invincible_timer >= true) {
		if (invincible_blinking%20 == 0 && sprite_exists(sprite_index)) {
			draw_sprite_ext(sprite_index, image_index, x, view_y + 32, draw_xscale * default_xscale * 0.5 * sign(image_xscale), draw_yscale * default_yscale * 0.5, angle, c_black, draw_arrow_outside_view_alpha);
		}
		else
		if (sprite_exists(sprite_index)) {
			draw_sprite_ext(sprite_index, image_index, x, view_y + 32, draw_xscale * default_xscale * 0.5 * sign(image_xscale), draw_yscale * default_yscale * 0.5, angle, image_blend, draw_arrow_outside_view_alpha);
		}
	}
	else
	if (sprite_exists(sprite_index)) {
...
&& (y > view_y + (view_height * 0.5)) {
	draw_set_alpha(draw_arrow_outside_view_alpha);
	draw_set_color(c_black);
	draw_arrow(x, view_center_y - 16, x, view_center_y + 2, 30);
	draw_set_color(c_white);
	draw_arrow(x, view_center_y - 16, x, view_center_y, 20);
	draw_circle_color(x, view_center_y - 32, 24, c_black, c_black, true);
	draw_circle_color(x, view_center_y - 32, 22, c_white, c_white, true);
	draw_set_alpha(1);
	
	if (!assist_invincible && invincible_timer >= true) {
		if (invincible_blinking%20 == 0 && sprite_exists(sprite_index)) {
			draw_sprite_ext(sprite_index, image_index, x, view_center_y - 32, draw_xscale * default_xscale * 0.5 * sign(image_xscale), draw_yscale * default_yscale * 0.5, angle, c_black, draw_arrow_outside_view_alpha);
		}
		else
		if (sprite_exists(sprite_index)) {
			draw_sprite_ext(sprite_index, image_index, x, view_center_y - 32, draw_xscale * default_xscale * 0.5 * sign(image_xscale), draw_yscale * default_yscale * 0.5, angle, image_blend, draw_arrow_outside_view_alpha);
		}
	}
	else
	if (sprite_exists(sprite_index)) {
...
if (taken_damage >= taken_damage_freezetime) {
	...
	vspeed = 0;
	speed = 0;
	if (sprite_die > noone && intro_animation == "") {
...
&& (intro_animation == "")
&& (invincible_timer == 0) {
	draw_sprite_ext(sprite_index, image_index, xx + random_range(- 8, + 8), yy + random_range(- 8, + 8), draw_xscale * default_xscale * sign(image_xscale), draw_yscale * default_yscale, angle, c_red, draw_alpha);
}
else
if (taken_damage%2 == 0 && intro_animation == "" && sprite_exists(sprite_index)) {
	draw_sprite_ext(sprite_index, image_index, xx, yy, draw_xscale * default_xscale * sign(image_xscale), draw_yscale * default_yscale, angle, image_blend, draw_alpha);
	if (hp <= 1 && max_hp >= 2 && invincible_timer == 0) {
...
if (intro_animation == "" && sprite_exists(sprite_index)) {
...
if (!assist_invincible) {
	if (invincible_timer >= true && sprite_exists(sprite_index)) {
		invincible_blinking ++;
		if (invincible_blinking > 21) {
			invincible_blinking = 0;
		}
		if (invincible_blinking%20 == 0) {
			draw_sprite_ext(sprite_index, image_index, xx, yy, draw_xscale * default_xscale * sign(image_xscale), draw_yscale * default_yscale, angle, c_black, draw_alpha);
		}
		else {
			...
			draw_sprite_ext(sprite_index, image_index, xx, yy, draw_xscale * default_xscale * sign(image_xscale), draw_yscale * default_yscale, angle, invincible_blend, draw_alpha);
		}
	}
}
