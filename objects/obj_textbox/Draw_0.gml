...
&& (string_length(textbox_text_masked) > 0)
{
	...
	{
		x_adjusted = camera_get_view_x(view_get_camera(view_current)) + (string_width_ext(textbox_text_masked, 40, 1000) * 0.4);
	}
	else
	...
	{
		x_adjusted = camera_get_view_x(view_get_camera(view_current)) + camera_get_view_width(view_get_camera(view_current)) - (string_width_ext(textbox_text_masked, 40, 1000) * 0.4);
	}
